...
class sitesettingsmodel extends CI_Model
{
	...
	public function updsitesettings()
	{
		...
		if($_FILES['sitelogo']['error']==0)
    		...
    		$sql="update ".tablename('site_settings')." set sitename='".$sitename."',siteemail='".$siteemail."',siteaddress='".$siteaddress."',fb_link='".$fb_link."',tw_link='".$tw_link."',gp_link='".$gp_link."',li_link='".$li_link."',ins_link='".$ins_link."',pin_link='".$pin_link."',youtube_link='".$youtube_link."',online_status='".$online_status."',commission='".$commission."',logback_color='".$logback_color."',base_color='".$base_color."',contrast_color='".$contrast_color."' where id='1'";
		$query=$this->db->query($sql);
		if(!empty($query))
		{
			return 1;
	...
	{
		$sql="select fb_link,tw_link,gp_link,li_link,ins_link,pin_link,youtube_link from ".tablename('site_settings')." where id='1'";
		$query=$this->db->query($sql);
		$row=$query->row();
		if(!empty($row))
		{
